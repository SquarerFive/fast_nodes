...
/// # FGraph
/// Represents a graph, which can be evaluated once or in parallel (see `grid3`).
///
///
///
///
...
///
/// ```
/// use five_incp_ark_node::vcnode::{FNode, FGraph, FGraphBase, ENodeType, compute, FNodeBase};
/// use std::collections::HashMap;
///
/// // Create our node graph:
/// let my_node_value = FNode::new (
///     "My Node 1".to_string(),
///     0,
///     None,
...
/// );
///  
/// let my_node_multiply = FNode::new (
///     "My Node 3".to_string(),
///     2,
///     Some(vec![0, 1]),
///     0.0,
...
/// );
///
/// // Declare the graph here
/// let mut graph = FGraph::new(vec![my_node_value, my_node_value_2, my_node_multiply], HashMap::new());  //FGraph {nodes: vec![my_node_value, my_node_value_2, my_node_multiply], nodes_pair: HashMap::new()};
/// // compute_fast requires the nodes to be allocated to the nodes_pair heap.
///
/// // sometimes we may want to change the value of the node
/// // also need to move the graph back into this scope. If we are using compute_fast, then change this to set_node_value_fast
/// graph = graph.set_node_value(0, 8.0);
///  
///  
/// let result:f64 = compute(&graph, &2);
///  
/// assert_eq!(result, 16.0);
/// ```
/// ### Grid 256^3
/// ```
/// use five_incp_ark_node::vcnode::{FNode, FGraph, FGraphBase, ENodeType, grid3, FNodeBase};
/// use std::collections::HashMap;
/// use ndarray::{Array3, Zip};
///
...
/// );
///
/// operator.expression = "(_i0 * _i1)".to_string();
///
/// let mut operator2 = FNode::new(
///     "My Operator 2".to_string(),
///     3,
...
/// );
///
/// operator2.expression = "(_i0 + _i1)".to_string();
///
/// let mut graph = FGraph::new(vec![constant1, constant2, operator, operator2], HashMap::new()); // {nodes: vec![constant1, constant2, operator, operator2], nodes_pair: HashMap::new()};
/// graph = graph.set_node_value(0, 8.0);
/// let val = grid3(&graph, &2, &16);
///         
/// assert_eq!(val[(0,0,0)], 64.0);
/// ```
///
#[derive(Debug, Clone)]
pub struct FGraph {
